...
public class ConsoleWriter : IWriter
{
    ...
    {
        foreach (var user in users) Console.WriteLine($"Id: {0} GivenName: {1} FamilyName: {2} Email: {2}", user.Id, user.GivenName, user.FamilyName, user.Email);
    }

    public void WriteOrders(IEnumerable<Order> orders)
    {
        foreach (var order in orders) Console.WriteLine($"Product: {0} User: {1} Quantity: {2} OccuredOn: {2}", order.ProductId, order.UserId, order.Quantity, $"{order.OccuredOn:R}");
    }
}